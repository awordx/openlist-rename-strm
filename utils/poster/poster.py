...
class LibraryPoster:
    ...
    async def async_get(self, url: str):
        ...
        async with self.session.get(url) as resp:
            ...
            if "application/json" in content_type:
                ...
                return await resp.read()  # 二进制内容（比如图片）


    async def async_post(self,url: str, data: Any, headers: dict[str, str] = None) -> aiohttp.ClientResponse:
        """
        异步 POST 请求
        :param url: 完整请求 URL
        :param data: POST 数据，可以是 str、bytes 或 dict
        :param headers: 可选请求头
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data, headers=headers) as resp:
                ...
                if "application/json" in content_type:
                    ...
                    return await resp.read()  # 原始二进制（可能是图片/文件）
    ...
    async def download_item_image(
        ...
        if resp is None:
            logger.warning(f"下载项目 {item['Name']} {image_type} 类型图片失败, 无响应")
            return None
        try:
    ...
    async def download_library_poster(
        ...
        resp = await self.get_library_items(library["Id"])
        for items in resp.get("Items", []):
            # filtered_results = [r for r in items if r]
            # if len(filtered_results) >= 13:
            #     items = random.sample(filtered_results, 13)
            # else:
    ...
    async def update_library_image(
        ...
        resp = await self.async_post(url, data=image_data_base64, headers=headers)
        if resp is None or resp.status != 204:
            # logger.warning(
            #     f"更新 {library['Name']} 媒体库图片失败, 状态码: {resp.status if resp else '无响应'}"
            # )
            pass
        else:
...
async def main(config):
    library_str = config['emby_library_config']['library']
    if not library_str:
        raise ValueError(f"海报媒体库配置文件格式错误！")
    library_list = ast.literal_eval(library_str)
    LibraryPosterList = \
        {
            "cron": config['emby_library_config']['cron'],
            "id": "Emby",
            "url": config['emby_config']['emby_url'],
            "api_key": config['emby_config']['api_key'],
            "title_font_path": r"utils/poster/fonts/ch.ttf",
            "subtitle_font_path": r"utils/poster/fonts/en.otf",
            "configs": library_list
        }
    scheduler = AsyncIOScheduler()
    poster_task = LibraryPoster(**LibraryPosterList)
    scheduler.add_job(poster_task.run, trigger=CronTrigger.from_crontab(LibraryPosterList['cron']))

    scheduler.start()
    # await poster_task.run()     #重启后运行一次
    await asyncio.Event().wait()#保持携程运行，避免调度器失效
if __name__ == "__main__":
    config = read_config(config_path)  # 目录问题修改working directory
    if 'emby_library_config' not in config:
        logger.error(f"配置文件缺少 [{'emby_library_config'}] 节")
        raise ValueError(f"配置文件缺少 [{'emby_library_config'}] 节，请更新到最新配置文件！")
    if eval(config['emby_library_config']['use_library']):
        asyncio.run(main(config))
    else:
        logger.info(f"媒体库海报刷新未开启，不进行刷新")
